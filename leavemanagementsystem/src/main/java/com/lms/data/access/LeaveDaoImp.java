...
public class LeaveDaoImp extends JdbcDaoSupport implements LeaveDao{
    ...
    @Override
    public int getLeaveCount(int month,int year) {

        String sql="select count(*) from leave where extract( month from leavedate)=? and extract( year from leavedate)=?";
        int monthlyLeaveCount = getJdbcTemplate().queryForObject(sql, new Object[]{month,year}, Integer.class);
